...
app.put("/api/update/LatestCheckupInfo", (req, res) => {
  ...
  const sqlUpdate =
    "UPDATE patient_checkup_info SET  chiefComplaint=?, historyofpatientIllness=?,generalAppearance=?, BP=?, HR=?, RR=?, temperature=?, o2Sat=?, weight=?, skin=?, heent=?, neck=?, chest=?, CVS=?, abdomen=?, gut=?, extromities=?, neuro=?, admittingDiagnosis=?, treatment=? WHERE patientId = ? AND dateOfAdmission = ?";
  db.query(
    sqlUpdate,
    [
      chiefComplaint,
...
app.delete("/api/Patient/delete/:id", (req, res) => {
...
app.delete("/api/Graph/delete/:id", (req, res) => {
  const id = req.params.id;
  console.log("I work");

  const sqlDelete = "DELETE FROM table_data WHERE id = ? ";
  db.query(sqlDelete, id, (err, result) => {
    if (err) {
      console.log(err);
    } else {
...
app.post("/api/PatientPersonalInfo/insert", (req, res) => {
  ...
  db.query(
    sqlInsert,
    [name, gender, dateofBirth, address, bloodType],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/api/TodoList/insert", (req, res) => {
  ...
  db.query(sqlInsert, [id, title, start, end, desc], (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.post("/api/Graph/insert", (req, res) => {
  const id = req.body.id;
  const Name = req.body.Name;
  ...
  db.query(
    sqlInsert,
    [id, Name, Executor, Age, Category, Cost, service],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/api/PatientCheckUpinfo/insert", (req, res) => {
  ...
  const date = Calculation.dateTime();
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        //validating with date in response
        res.send(date);
      }
    }
  );
...
app.get("/api/get/TodoList", (req, res) => {
  const sqlSelect = "SELECT * FROM time_data";
  db.query(sqlSelect, (err, result) => {
    if (err) {
      console.log("I work5");
      console.log(err);
...
app.get("/api/get/Graph", (req, res) => {
  const sqlSelect = "SELECT * FROM table_data";
  db.query(sqlSelect, (err, result) => {
    if (err) {
      console.log("I work5");
      console.log(err);
...
app.get("/api/get/DoctorList", (req, res) => {
  const sqlSelect = "SELECT * FROM doctor_data";
  db.query(sqlSelect, (err, result) => {
    if (err) {
      console.log("I work5");
      console.log(err);
...
app.get("/api/get/CheckupInfo/Details/:id/:date", (req, res) => {
  ...
  const sqlSelect =
    "SELECT * FROM patient_checkup_info WHERE patientId = ? AND dateOfAdmission = ?";
...
app.get("/api/get/dateList/:id", (req, res) => {
  ...
  const sqlSelect =
    "SELECT dateOfAdmission FROM patient_checkup_info WHERE patientId=?";
...
function updateDateOfLatestAdmission(patientId, date) {
  const sqlUpdate =
    "UPDATE patient_data SET dateOfLatestAdmission = ? WHERE id = ?";
